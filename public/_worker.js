...
.url-box {
    ...
    word-break: break-all; text-align: left; margin-bottom: 12px;
}
.copy-btn {
    display: block; width: 100%; background: #1a73e8; color: white;
    border: none; border-radius: 8px; padding: 13px;
    font-size: 15px; cursor: pointer; font-weight: 500;
    font-family: inherit; transition: background 0.2s;
}
.copy-btn:active { background: #1557b0; }
...
<div class="card">
    ...
    <div class="steps">
        <div class="step">
            ...
            <div class="step-text">点击下方「复制链接」按钮</div>
        </div>
        <div class="step">
            <div class="step-num">2</div>
            <div class="step-text">打开手机浏览器（Safari / Chrome / 系统浏览器）</div>
        </div>
        <div class="step">
            <div class="step-num">3</div>
            <div class="step-text">粘贴到地址栏即可正常访问</div>
        </div>
    </div>
...
export default {
    async fetch(request, env) {
        ...
        // 跳过静态资源，避免样式/图片被拦截
        const isStatic = /\.(css|js|png|jpg|jpeg|gif|svg|ico|woff|woff2|ttf|webp|avif)$/i.test(url.pathname);

        // 检测微信/QQ UA
        const isBlocked = /MicroMessenger|QQ\//i.test(ua);

        if (isBlocked && !isStatic) {
            const blockUrl = `${url.origin}/__wechat-block?url=${encodeURIComponent(request.url)}`;
            // 如果已经在拦截页，直接放行避免死循环
            if (url.pathname === '/__wechat-block') {
                return new Response(BLOCK_PAGE.replace('加载中...', decodeURIComponent(url.searchParams.get('url') || request.url)), {
